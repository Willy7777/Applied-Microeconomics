...
chartSeries(num_of_volume_Moderna[, 1], theme = "white" , name = "positive")
addBBands(n = 7, sd = 1.96, maType = "EMA")   # 95 %
addEMA(n = 7, col = "blue")


# BBands's up and down
bb <- BBands(num_of_volume_Moderna[, 1], sd = 1.96, n = 7, maType = "EMA")
bb$positive <- num_of_volume_Moderna$positive
bb$over_bb_up <- 0
bb$over_bb_dn <- 0

for (i in 7:length(bb$positive)) {
  if (bb$positive[[i]] > bb$up[[i]]) {
    bb$over_bb_up[[i]] <- 1
  } else if (bb$positive[[i]] < bb$dn[[i]]) {
    bb$over_bb_dn[[i]] <- 1
  }
}
...
write.csv(temp, "Moderna_BBands_numerical_positive.csv")
rm(temp)
